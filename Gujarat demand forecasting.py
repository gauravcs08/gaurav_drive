...
def decompse_data(ts):
    decomposition=seasonal_decompose(ts,model='addative')
...
def HLM_winter_model(train,test):
    ...
    fit1 = ExponentialSmoothing(train,seasonal_periods=252,trend='mul',seasonal='mul',damped=False).fit(optimized=True, use_boxcox=False, remove_bias=True)
    fcast=fit1.forecast(len(test))
    plt.figure(figsize=(18,8))
    plt.plot(train,label='train data',color='black')
    plt.plot(test,label='test data',color='green')
...
def test_stationarity(ts):
    ...
    ax1.plot(rolstd,color='black',label='STD')
    plt.legend(loc='best')
...
def ARIMA_model(train,test,p,i,q):
     model=ARIMA(np.log(train),order=(p,i,q))
     res=model.fit()
     print(res.summary())
...
def srimax_model(train,test,p,i,q,sp,si,sq):
     model=sm.tsa.statespace.SARIMAX(train,order=(p,i,q),seasonal_order=(sp,si,sq,148),enforce_stationarity=False,enforce_invertibility=False)
